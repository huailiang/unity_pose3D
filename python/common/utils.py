...
def add_path():
    ...
    paths = filter(lambda p: p not in sys.path, [Alphapose_path,  trackers_path])
...
class Timer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.show:
            print('{self.message} --- elapsed time: {time.perf_counter() - self.start} s')
...
def split_video(video_path):
    ...
    save_folder = pathlib.Path('./{0}/alpha_pose_{1}/split_image/'.format(output_dir, video_name))
    ...
    while True:
        ...
        if not grabbed:
            print('Split totally {i + 1} images from video.')
            break
    ...
    print('Split images saved in {saved_path}')
